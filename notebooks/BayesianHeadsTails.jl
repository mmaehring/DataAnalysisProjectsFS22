...
# ╔═╡ 47bdc69f-f551-44c2-b0ec-05e82463c51e
using Plots, PlutoUI, Plots, Distributions

# ╔═╡ e8165e00-26a6-11ed-1cc6-d5a84febc81d
begin
	## Interactivity
	FLIP_disp = @bind flips Slider(vcat(0:4, [2^i for i in 3:1:12]), show_value=true)
	show_true_value = @bind show_val CheckBox()
	
	Uniform_p(x) = pdf(Uniform(), x)
	#Arcsine_p(x) = pdf(Arcsine(), x)
	Normal_p(x) = ifelse(0 <= x <= 1,  pdf(Normal(0.5, 0.05), x), 0)
	
	choose_prior = @bind prior Select([Uniform_p, Normal_p]) #Arcsine_p
	
	## Display
	md"""
	You are in Las Vegas, and encounter a strange coin. You want to test whether it is fair or not using Bayesian statistics. 
	...
	**Show true value:**
	...
	The first four flips are predetermined as "H" "H" "T" "H".
	"""
end

# ╔═╡ b4df98dc-bee5-4061-8656-eb150e87e89d
begin
	## Data generation
	#=
	H -> 1
	...
	posterior(x) = prior(x) .* distr(x)
	
	## Plotting
	xvec = 0:0.001:1
	yvec = posterior.(BigFloat.(xvec))
	yvec = yvec ./ maximum(yvec)
	
	plt = plot(xvec, yvec, label = "",
		xlabel="Bias-weighting for heads H",
		ylabel="prob(H|{data}, I)",
		xlims=(0.0, 1.0), ylims=(0.0, 1.1),
		xticks=0:0.1:1,
		minorgrid=true
	)
	if show_val
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "c9583c6f8a4ac35bb34e222770ebdaeaa2f97688"
